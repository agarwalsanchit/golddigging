...
select c.group,price, avg(ocgroup.quant_ordered) as Average_orders
from catalogue c 
left join  
(
...
Select location as City, sum(rev) as TOTAL_REV 
from  
( 
...
Select F.*, (Returned/Total_Sold*100) as Return_Percentage 
from  
( 
    Select case  
    when extract(month from date)=4 then'April' 
